...
def get_sha256_from_zip(path)
  ...
  Zip::File.open(path) do |zip_file|
    zip_file.each do |entry|
      ...
      entry_name = entry.name.sub(/^#{prefixed_directory.to_s}/,"")
      crc_sum << [entry_name, entry.crc] unless entry_name.empty?
      first_record = false
    end
  end
...
def fetch_package(package_options, package_file_path, package_name = "UNKNOW")
  if package_options[:type] == "gh-release"
    ...
    addon_xml_content = get_file_content_from_git(git_repo, package_options[:tag], "addon.xml")
    addon_detail = get_addon_info_from_xml(addon_xml_content)
    $logger.info("Creating addon package [#{package_name}]")
    git_archive(git_repo, "HEAD", package_file_path, package_options[:addon_id])
